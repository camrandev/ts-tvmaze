...
const $episodesList = jquery__WEBPACK_IMPORTED_MODULE_1__("#episodesList");
const $showEpisodesButton = jquery__WEBPACK_IMPORTED_MODULE_1__(".Show-getEpisodes");
const DEFAULT_IMAGE_URL = "https://store-images.s-microsoft.com/image/apps.65316.13510798887490672.6e1ebb25-96c8-4504-b714-1f7cbca3c5ad.f9514a23-1eb8-4916-a18e-99b1a9817d15?mode=scale&q=90&h=300&w=300";
...
async function getShowsByTerm(term) {
    const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().get(`${BASE_URL}/search/shows?q=${term}`);
    //format data for client
    const formattedShows = response.data.map((item) => {
...
async function getEpisodesOfShow(id) {
    const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().get(`${BASE_URL}/shows/${id}/episodes`);
    response;
    const formattedEpisodes = response.data.map((episode) => {
 ...
 */
function populateEpisodes(formattedEpisodes) {
    $episodesList.empty();
    for (let episode of formattedEpisodes) {
        const $episode = jquery__WEBPACK_IMPORTED_MODULE_1__(`<li>${episode.name} (${episode.season}, ${episode.number})</li>`);
...
async function handleClick(evt) {
    const $show = jquery__WEBPACK_IMPORTED_MODULE_1__(evt.target.closest(".Show"));
